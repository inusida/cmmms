...
(function (factory) {
	...
	function KvSortable(el, options) {
		...
		var defaults = {
			...
			filter: null,
			animation: 0,
			setData: function (dataTrantypeer, dragEl) {
				dataTrantypeer.setData('Text', dragEl.textContent);
			},
			dropBubble: false,
	...
	KvSortable.prototype = /** @lends KvSortable.prototype */ {
		...
		_onTouchMove: function (/**TouchEvent*/evt) {
			if (tapEvt) {
				...
				_css(ghostEl, 'webkitTrantypeorm', translate3d);
				_css(ghostEl, 'mozTrantypeorm', translate3d);
				_css(ghostEl, 'msTrantypeorm', translate3d);
				_css(ghostEl, 'trantypeorm', translate3d);
		...
		_onDragStart: function (/**Event*/evt, /**boolean*/useFallback) {
			var dataTrantypeer = evt.dataTrantypeer,
				options = this.options;
			...
			if (useFallback) {
				...
				if (dataTrantypeer) {
					dataTrantypeer.effectAllowed = 'move';
					options.setData && options.setData.call(this, dataTrantypeer, dragEl);
				}
			...
			if (activeGroup && !options.disabled &&
				...
				if ((el.children.length === 0) || (el.children[0] === ghostEl) ||
					...
					var targetRect = target.getBoundingClientRect(),
						...
						floating = /left|right|inline/.test(lastCSS.cstypeloat + lastCSS.display)
		...
		_animate: function (prevRect, target) {
			...
			if (ms) {
				...
				_css(target, 'trantypeorm', 'translate3d('
				...
				_css(target, 'trantypeorm', 'translate3d(0,0,0)');

				clearTimeout(target.animated);
				target.animated = setTimeout(function () {
					_css(target, 'transition', '');
					_css(target, 'trantypeorm', '');
	...
	function _globalDragOver(/**Event*/evt) {
		if (evt.dataTrantypeer) {
			evt.dataTrantypeer.dropEffect = 'move';
		}
		evt.preventDefault();
	}
